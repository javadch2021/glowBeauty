...
export class OrderService {
  ...
  async getByCustomerId(customerId: number): Promise<Order[]> {
    try {
      const collection = await this.getCollection();
      const documents = await collection.find({ customerId }).sort({ id: -1 }).toArray();
      return documents.map(documentToOrder);
    } catch (error) {
  ...
  async getByStatus(status: Order['status']): Promise<Order[]> {
    try {
      const collection = await this.getCollection();
      const documents = await collection.find({ status }).sort({ id: -1 }).toArray();
      return documents.map(documentToOrder);
    } catch (error) {
   ...
   * Update order status
   */
  async updateStatus(id: number, status: Order['status'], trackingNumber?: string): Promise<Order> {
    try {
      const collection = await this.getCollection();
  ...
  async delete(id: number): Promise<boolean> {
    try {
      ...
      if (deleted) {
        ...
        console.log(`OrderService - Order with ID ${id} not found for deletion`);
      }
   ...
   */
  async getPaginated(page: number = 1, limit: number = 10): Promise<{ orders: Order[], total: number, page: number, totalPages: number }> {
    try {
      const collection = await this.getCollection();
