...
export class CustomerService {
  ...
  async delete(id: number): Promise<boolean> {
    try {
      ...
      if (deleted) {
        ...
        console.log(
          `CustomerService - Customer with ID ${id} not found for deletion`
        );
      }
  ...
  async updateStats(
    ...
    try {
      ...
      if (result.matchedCount === 0) {
        console.warn(
          `Customer with ID ${customerId} not found for stats update`
        );
      } else {
        console.log(
          `CustomerService - Updated stats for customer ${customerId}`
        );
      }
    } catch (error) {
      console.error(
        `Error updating customer stats for ID ${customerId}:`,
        error
      );
