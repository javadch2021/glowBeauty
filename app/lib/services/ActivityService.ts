...
export class ActivityService {
  ...
  async getRecent(limit: number = 10): Promise<ActivityItem[]> {
    try {
      const collection = await this.getCollection();
      const documents = await collection.find({}).sort({ id: -1 }).limit(limit).toArray();
      return documents.map(documentToActivity);
    } catch (error) {
  ...
  async getByType(type: ActivityItem['type']): Promise<ActivityItem[]> {
    try {
      const collection = await this.getCollection();
      const documents = await collection.find({ type }).sort({ id: -1 }).toArray();
      return documents.map(documentToActivity);
    } catch (error) {
   ...
   * Log order activity
   */
  async logOrderActivity(orderId: number, customerName: string, action: string): Promise<void> {
    try {
   ...
   */
  async logCustomerActivity(customerName: string, action: string): Promise<void> {
    try {
  ...
  async cleanup(keepCount: number = 100): Promise<number> {
    try {
      ...
      const itemsToKeep = await collection.find({}).sort({ id: -1 }).limit(keepCount).toArray();
      
      if (itemsToKeep.length === keepCount) {
        const oldestToKeepId = itemsToKeep[itemsToKeep.length - 1].id;
        const result = await collection.deleteMany({ id: { $lt: oldestToKeepId } });
        
        console.log(`ActivityService - Cleaned up ${result.deletedCount} old activity items`);
   ...
   */
  async getPaginated(page: number = 1, limit: number = 10): Promise<{ activities: ActivityItem[], total: number, page: number, totalPages: number }> {
    try {
      const collection = await this.getCollection();
  ...
  async deleteAll(): Promise<number> {
    try {
      ...
      console.log(`ActivityService - Deleted all ${result.deletedCount} activity items`);
      return result.deletedCount;
    } catch (error) {
